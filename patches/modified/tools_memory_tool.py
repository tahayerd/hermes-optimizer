...
class MemoryStore:
    ...
    def select_for_query(self, target: str, query: str, top_k: int = 5) -> Optional[str]:
        """Return top-k entries relevant to *query* using TF-IDF similarity."""
        if not query or not query.strip():
            return self.format_for_system_prompt(target)
        entries = self._entries_for(target)
        if not entries:
            return None
        from agent.memory_router import tf_idf_rank_indices
        indices = tf_idf_rank_indices(query, entries, top_k=top_k)
        selected = [entries[i] for i in indices]
        return self._render_block(target, selected)

    def get_target_descriptions(self) -> dict:
        """Return short descriptions of available memory targets for routing."""
        return {
            "memory": "Agent's persistent memory notes about user, project, and conventions",
            "user": "User profile information and preferences",
        }
