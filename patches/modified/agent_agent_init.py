...
def init_agent(
    ...
    agent._selective_retrieval = False
    agent._memory_router = None
    if not skip_memory:
        try:
            ...
            if agent._memory_enabled or agent._user_profile_enabled:
                ...
                agent._selective_retrieval = bool(mem_config.get("selective_retrieval", False))
                agent._memory_router = None
                if agent._selective_retrieval:
                    from agent.memory_router import MemoryRouter
                    agent._memory_router = MemoryRouter(
                        agent._memory_store,
                        top_k=int(mem_config.get("selective_top_k", 5)),
                    )
        except Exception:
