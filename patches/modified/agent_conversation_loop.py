...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # [hermes-optimizer] Selective memory retrieval: inject per-turn
        if getattr(agent, "_selective_retrieval", False) and getattr(agent, "_memory_router", None):
            _router_blocks = agent._memory_router.route(user_message)
            for _target in sorted(_router_blocks.keys()):
                _block = _router_blocks[_target]
                if _block:
                    api_messages.insert(1, {"role": "system", "content": _block})
